...
def manually_fill():
    ...
    def fill_attendance():
        ...
        #---Code for Creating table for Attendance
        ...
        try:
            global cursor
            connection = pymysql.connect(host='localhost', user='root', password='Shyam@123', db='manually_fill_attendance')
        ...
        if subb=='':
            ...
            ENR = tk.Label(MFW, text="Enter Enrollment", width=15, height=2, fg="black", bg="pink2",
                           font=('times', 15, ' bold '))
            ENR.place(x=30, y=100)

            STU_NAME = tk.Label(MFW, text="Enter Student name", width=15, height=2, fg="black", bg="pink2",
                                font=('times', 15, ' bold '))
            ...
            def create_csv():
                ...
                csv_name='/Users/kajalkumari/Desktop/Mark/Attendance/ManuallyAttendance/'+DB_table_name+'.csv'
                with open(csv_name, "w") as csv_file:
                    csv_writer = csv.writer(csv_file)
                    csv_writer.writerow([i[0] for i in cursor.description])  # write headers
                    csv_writer.writerows(cursor)
                    O="CSV created Successfully"
                    Notifi.configure(text=O, bg="pink", fg="black", width=33, font=('times', 19, 'bold'))
            ...
            Notifi = tk.Label(MFW, text="CSV created Successfully", bg="pink", fg="black", width=33,
            ...
            def attf():
                import subprocess
                subprocess.Popen("/Users/kajalkumari/Desktop/Mark/Attendance/ManuallyAttendance/")
    ...
    SUB = tk.Label(sb, text="Enter Subject", width=15, height=2, fg="white", bg="pink2", font=('times', 15, ' bold '))
...
##Code For taking images for datasets
def take_img():
    l1 = txt.get()
    l2 = txt2.get()
    if l1 == '':
        ...
        try:
            ...
            with open('/Users/kajalkumari/Desktop/Mark/StudentDetails/StudentDetails.csv', 'a+') as csvFile:
                writer = csv.writer(csvFile, delimiter=',')
                writer.writerow(row)
                csvFile.close()
            res = "Images Saved for Enrollment : " + Enrollment + " Name : " + Name
            Notification.configure(text=res, bg="pink", width=50, font=('times', 18, 'bold'))
...
###Code for choosing subject and fill attendance
def subjectchoose():
    def Fillattendances():
        sub=tx.get()
        ...
        if time.time() < future:
            if sub == '':
                ...
                df = pd.read_csv("/Users/kajalkumari/Desktop/Mark/StudentDetails/StudentDetails.csv")
                cam = cv2.VideoCapture(0)
                ...
                ###----Code for Connect to the database
                try:
                    global cursor
                    connection = pymysql.connect(host='localhost', user='root', password='Shyam@123', db='Face_reco_fill')
                    cursor = connection.cursor()
                except Exception as e:
                    print(e)
                ...
                Notifica.configure(text=M, bg="pink", fg="white", width=33, font=('times', 15, 'bold'))
                Notifica.place(x=20, y=250)
                ...
                root.configure(background='snow')
                cs = '/Users/kajalkumari/Desktop/Mark/' + fileName
    ...
    windo = tk.Tk()
    windo.iconbitmap('Mark.ico')
    windo.title("Enter subject name...")
    windo.geometry('580x320')
    windo.configure(background='snow')
    Notifica = tk.Label(windo, text="Attendance filled Successfully", bg="pink", fg="white", width=33,
    ...
    sub = tk.Label(windo, text="Enter Subject", width=15, height=2, fg="black", bg="pink2", font=('times', 15, ' bold '))
    sub.place(x=30, y=100)
...
def admin_panel():
    ...
    def log_in():
        ...
        if username == 'kajal' :
            if password == 'kajal2303':
                ...
                cs = '/Users/kajalkumari/Desktop/Mark/StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
                ...
                Nt.configure(text=valid, bg="pink", fg="black", width=38, font=('times', 19, 'bold'))
    ...
    un = tk.Label(win, text="Enter username", width=15, height=2, fg="black", bg="pink2",
                   font=('times', 15, ' bold '))
    un.place(x=30, y=50)

    pw = tk.Label(win, text="Enter password", width=15, height=2, fg="black", bg="pink2",
                  font=('times', 15, ' bold '))
    pw.place(x=30, y=150)
...
def trainimg():
    ...
    try:
        ...
        Notification.configure(text=l, bg="pink", width=50, font=('times', 18, 'bold'))
    ...
    try:
        ...
        Notification.configure(text=q, bg="pink", width=50, font=('times', 18, 'bold'))
        Notification.place(x=350, y=400)
...
message = tk.Label(window, text="Face-Recognition-Based-Attendance-Management-System", bd = 5, bg="pink", fg="black", width=50,
...
lbl = tk.Label(window, text="Enter Enrollment", width=20, height=2, bd = 3 ,fg="black", bg="pink", font=('times', 15, ' bold '))
...
txt = tk.Entry(window, validate="key", width=20, bg="white", bd=3,fg="black", font=('times', 25, ' bold '))
txt['validatecommand'] = (txt.register(testVal),'%P','%d')
txt.place(x=550, y=210)

lbl2 = tk.Label(window, text="Enter Name", width=20, fg="black", bg="pink", bd = 3, height=2, font=('times', 15, ' bold '))
lbl2.place(x=200, y=300)

txt2 = tk.Entry(window, width=20, bd = 3, bg="white", fg="black", font=('times', 25, ' bold '))
...
takeImg = tk.Button(window, text="Take Images",command=take_img,fg="black"  ,bg="pink"  ,width=20  ,height=3, activebackground = "Red" ,font=('times', 15, ' bold '))
takeImg.place(x=90, y=500)

trainImg = tk.Button(window, text="Train Images",fg="black",command=trainimg ,bg="pink"  ,width=20  ,height=3, activebackground = "Red" ,font=('times', 15, ' bold '))
trainImg.place(x=390, y=500)

FA = tk.Button(window, text="Automatic Attendace",fg="black",command=subjectchoose  ,bg="pink"  ,width=20  ,height=3, activebackground = "Red" ,font=('times', 15, ' bold '))
FA.place(x=690, y=500)

quitWindow = tk.Button(window, text="Manually Fill Attendance", command=manually_fill  ,fg="black"  ,bg="pink"  ,width=20  ,height=3, activebackground = "Red" ,font=('times', 15, ' bold '))
